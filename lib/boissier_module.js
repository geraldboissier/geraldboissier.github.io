...
var BOISSIER = (function(){
  ...
  var changeHeaderImage = function(){
    ...
    // Get image url
    var actualImageUrl = $('#header_image').css('background-image').replace("\"","").replace("\"","");

    // Extract image name
    var start = actualImageUrl.lastIndexOf("/")+1;
    var end   = actualImageUrl.length-1;
  ...
  publicAPI.initHeaderLeftMove = function(){
    ...
    var x = window.innerWidth/2*(-1) + 190;
    var y = window.innerHeight/2*(-1) + 150;

    x = x < 0 ? x+'px': '0px';
    y = y < 0 ? y+'px': '0px';

    var cssValue = 'translate('+x+','+y+') scale(0.60,0.60)';
